...
static void _update_status(uint8_t status) {
	...
	if (!(last & CdlStatError) && (status & CdlStatError))
		printf("drive error, status=0x%02x, code=0x%02x\n", _last_status, _last_error);
...
static void _cd_irq_handler(void) {
	CDROM_REG(0) = 1;
	CdlIntrResult irq = CDROM_REG(3) & 7;
...
int CdInit(void) {
	...
	if (CdSync(0, 0) == CdlDiskError) {
		printf("setup error, bad disc/drive or no disc inserted\n");
		return 0;
	}
...
int CdCommand(CdlCommand cmd, const void *param, int length, uint8_t *result) {
	...
	/*if (_ack_pending) {
		printf("CdCommand(0x%02x) failed, drive busy\n", cmd);
		return 0;
	}*/
	...
	printf("CdCommand(0x%02x) failed, acknowledge timeout\n", cmd);
	return 0;
}
...
int CdControlF(CdlCommand cmd, const void *param) {
	...
	if (flags & OPTIONAL) {
		...
		length = flags & 3;
		if (length && !param) {
			printf("CdControl() param is required for command 0x%02x\n", cmd);
			return -1;
		}
	}
...
int CdControl(CdlCommand cmd, const void *param, uint8_t *result) {
	/*if (_ack_pending) {
		printf("CdControl(0x%02x) failed, drive busy\n", cmd);
		return 0;
	}*/
	...
	printf("CdControl(0x%02x) failed, acknowledge timeout\n", cmd);
	return 0;
}
