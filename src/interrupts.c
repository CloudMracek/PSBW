...
void *InterruptCallback(IRQChannel irq, void (*func)(void)) {
...
void *GetInterruptCallback(IRQChannel irq) {

	return _irq_handlers[irq];
}

void *DMACallback(DMAChannel dma, void (*func)(void)) {
...
void *GetDMACallback(DMAChannel dma) {
	return _dma_handlers[dma];
}

/* DMA channel priority API */

int SetDMAPriority(DMAChannel dma, int priority) {
...
int GetDMAPriority(DMAChannel dma) {
